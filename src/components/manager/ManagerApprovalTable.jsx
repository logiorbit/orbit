...
import { getLeavesForManagerApproval } from "../../services/sharePointService";
...
export default function ManagerApprovalTable() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const data = await getLeavesForManagerApproval(token);

        const myEmail = userProfile.email.toLowerCase();

        // ✅ ONLY Manager-related leaves
        const myTeamLeaves = data.filter((leave) => {
          ...
          return hierarchy.Manager?.EMail?.toLowerCase() === myEmail;
        });
  ...
  if (loading) {
    return <p className="muted">Loading manager approvals...</p>;
  }

  /* ---------- TABLE CONFIG (SAME AS TL) ---------- */
  ...
  const tableData = leaves.map((l) => ({
    ...
    Reason: l.Reason,
    Status: l.Status,
    _raw: l, // keep original SP record for modal
  }));
