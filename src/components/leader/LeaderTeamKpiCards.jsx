...
export default function LeaderTeamKpiCards() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const today = await getAllTasksForPeriod(token, "today");

      const week = await getAllTasksForPeriod(token, "week");

      const month = await getAllTasksForPeriod(token, "month");
